const materialColors = {
    ...
    function copyColor(hex, name) {
      navigator.clipboard.writeText(hex).then(() => {
        ...
        toast.textContent = `Copied ${name}: ${hex}`;
    ...
    function renderPalettes() {
      ...
      grid.innerHTML = Object.entries(materialColors).map(([colorName, shades]) => {
        const mainShades = Object.entries(shades).filter(([shade]) => !shade.startsWith('A'));
        const accentShades = Object.entries(shades).filter(([shade]) => shade.startsWith('A'));
        
        const shadesHtml = mainShades.map(([shade, hex]) => {
          const textColor = getContrastColor(hex);
          return `
            <div class="shade-item" style="background: ${hex}; color: ${textColor}" 
                 onclick="copyColor('${hex}', '${colorName} ${shade}')">
              <div class="shade-label">${shade}</div>
              <div class="shade-hex">${hex}</div>
            </div>
          `;
        }).join('');
        
        const accentsHtml = accentShades.length > 0 ? `
          <div class="accents-grid">
            ${accentShades.map(([shade, hex]) => {
              const textColor = getContrastColor(hex);
              return `
                <div class="accent-item" style="background: ${hex}; color: ${textColor}"
                     onclick="copyColor('${hex}', '${colorName} ${shade}')">
                  <div class="accent-label">${shade}</div>
                  <div class="shade-hex">${hex}</div>
                </div>
              `;
            }).join('')}
        ...
        return `
          <div class="palette-section">
            <div class="palette-header">
              <div class="palette-name">${colorName.replace(/([A-Z])/g, ' $1').trim()}</div>
            </div>
